...
public class Concord {
    ...
    public void printSorted(PrintStream out) {
        ...
        for (Map.Entry<String, Integer> curr : concord.entrySet()) {
            ...
            newValues.add(curr.getValue());
    	}

        Collections.sort(newValues, Collections.reverseOrder());

        Map<String, Integer> newConcord = new TreeMap<String, Integer>(concord);

        int i=0;
        while(i<newValues.size()){
            Iterator<Map.Entry<String, Integer>> iter = newConcord.entrySet().iterator();
            while(iter.hasNext()){
                Map.Entry<String, Integer> curr = iter.next();

                if(curr.getValue() == newValues.get(i)){
                    out.println(curr.getKey() + " " + curr.getValue());
                    newConcord.remove(curr.getKey());
                    i++;
                    break;
                }
            }

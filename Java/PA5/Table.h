...
public:
  ...
  static const int HASH_SIZE = 9973;  // a prime number
   ...
   // change a score from a pair in the table
   // return false iff this key was not in the table
   //         (and no change made to table)
...
private:
  ...
  unsigned int hashCode(const string &word) const {
    ...
    ListType *data;             // declare a Node ** table (hash table)
    int numEntry;               // keep track of the number of entries
